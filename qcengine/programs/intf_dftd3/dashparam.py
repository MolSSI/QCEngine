...
dashcoeff = {
    'd2': {
        ...
        'definitions': {
            ...
            'revpbe'        : {'params': {'s6': 1.25, 'alpha6': 20.0, 'sr6': 1.1}, "citation": '    S. Grimme, J. Antony, S. Ehrlich, H. Krieg, J. Chem. Phys 132, 154104, 2010\n'},
            ...
            'tpss0'         : {'params': {'s6': 0.85, 'alpha6': 20.0, 'sr6': 1.1}, "citation": '    S. Grimme, J. Antony, S. Ehrlich, H. Krieg, J. Chem. Phys 132, 154104, 2010\n'},
    ...
    'd3zero': {
        ...
        # Later references
            'rpw86pbe'    : {'params': {'s6': 1.0,  's8': 0.901,  'sr6': 1.224,  'alpha6': 14.0, 'sr8': 1.000}, "citation": '    S. Grimme, S. Ehrlich, L. Goerigk, J. Comput. Chem. 32, 1456-1465, 2011\n'},
    ...
    'd3bj': {
        ...
        # Later references
            'bpbe'          : {'params': {'s6': 1.000, 's8':  4.0728, 'a1':  0.4567, 'a2': 4.3908}, "citation": '    L. Goerigk, S. Grimme, Phys. Chem. Chem. Phys. 13, 6670-6688, 2011\n'},
    ...
    'nl': {
        ...
        'definitions'     : {
            'blyp'        : {'params': {'b':  4.000, 'c': 0.0093}, 'citation' : '    W. Hujo, S. Grimme J. Chem. Theory Comput. 7, 3866-3871, 2011\n'},
            'hf'          : {'params': {'b':  3.900, 'c': 0.0093}, 'citation' : '    W. Hujo, S. Grimme J. Chem. Theory Comput. 7, 3866-3871, 2011\n'},  # not implemented
            'revpbe'      : {'params': {'b':  3.700, 'c': 0.0093}, 'citation' : '    W. Hujo, S. Grimme J. Chem. Theory Comput. 7, 3866-3871, 2011\n'},
            ...
            'pbe0'        : {'params': {'b':  6.900, 'c': 0.0093}, 'citation' : '    M. K. Kesharwani, A. Karton, J.M. L. Martin, J. Chem. Theory Comput. 12, 444-454, 2016\n'},  # unclear if this is the real origin
            'pbe'         : {'params': {'b':  6.400, 'c': 0.0093}, 'citation' : '    M. K. Kesharwani, A. Karton, J.M. L. Martin, J. Chem. Theory Comput. 12, 444-454, 2016\n'},  # unclear if this is the real origin
            'tpss0'       : {'params': {'b':  5.500, 'c': 0.0093}, 'citation' : '    W. Hujo, S. Grimme, J. Chem. Theory Comput. 9, 308-315, 2013\n'},
...
def from_arrays(name_hint=None, level_hint=None, param_tweaks=None, dashcoeff_supplement=None, verbose=1):
    ...
    * Function intended to be idempotent.
        ...
        for disp, ddisp in dashcoeff_supplement.items():
            for func, params in ddisp['definitions'].items():
                if params['params'].keys() != dashcoeff[disp]['default'].keys():
                    if verbose > 2:
                        print('Warning: trouble in dict_builder def:', func, params['params'].keys(), '!=',
    ...
    if name_hint is None and level_hint is None:
        raise ValidationError("""Can't guess -D level without name_hint ({}) or level_hint ({})""".format(
    ...
    if name_hint is None:
        ...
        #    disp_params = dashcoeff[dashlevel_candidate_2]['definitions'][name_key]['params']
            ...
            dashlevel_candidate_2 = None
            raise ValidationError("""Can't guess -D correction level from ({})""".format(name_hint))
    ...
    if name_hint is None and param_tweaks is None:
        raise ValidationError("""Can't guess -D parameters without name_hint ({}) or param_tweaks ({})""".format(
    ...
    if not set(param_tweaks.keys()).issubset(allowed_params):
        raise ValidationError('Requested keys ({}) not among allowed ({}) for dispersion level ({})'.format(
            list(param_tweaks.keys()), list(allowed_params), dashleveleff))

    disp_params.update(param_tweaks)

    if disp_params.keys() != allowed_params:
        raise ValidationError('Requested keys ({}) insufficient ({}) for dispersion level ({})'.format(
            list(param_tweaks.keys()), list(allowed_params), dashleveleff))
