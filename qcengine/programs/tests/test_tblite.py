"""
Testing for the tblite program harness.

Tests cover GFN1-xTB, GFN2-xTB, and IPEA1-xTB methods using mindless
molecules for the diversity of element species to test different interactions.
"""
...
_KtoEh = qcel.constants.conversion_factor("kelvin", "hartree")

@uusing("tblite")
def test_tblite_task_gfn1xtb_m01(schema_versions, request):
    ...
    atomic_result = qcng.compute(atomic_input, "tblite", return_version=retver)
    atomic_result = checkver_and_convert(atomic_result, request.node.name, "post")
    pprint.pprint(atomic_result.model_dump(), width=200)
...
@uusing("tblite")
def test_tblite_task_gfn1xtb_m02(schema_versions, request):
    models, retver, _ = schema_versions
...
# TODO
@uusing("tblite")
def hide_test_tblite_task_gfn1xtb_m03(schema_versions, request):
    models, retver, _ = schema_versions
        ...
        atomic_input = models.AtomicInput(
            ...
            specification=models.AtomicSpecification(
                ...
                keywords={
                    ...
                    #"alpb-solvation": ["chcl3", "reference"],
                   #{"alpb-solvation": ["water", "bar1mol"]}
                },
            ),
        )
...
@uusing("tblite")
def test_tblite_task_gfn1xtb_m04(schema_versions, request):
    models, retver, _ = schema_versions
    ...
    assert atomic_result.success
    assert pytest.approx(return_result["dipole"], abs=thr) == atomic_result.return_result["dipole"]
    assert pytest.approx(return_result["mulliken_charges"], abs=thr) == atomic_result.return_result["mulliken_charges"]
    assert "mayer_indices" in atomic_result.return_result


@uusing("tblite")
def test_tblite_task_gfn1xtb_m05(schema_versions, request):
    models, retver, _ = schema_versions
...
@uusing("tblite")
def test_tblite_task_gfn2xtb_m01(schema_versions, request):
    models, retver, _ = schema_versions
...
@uusing("tblite")
def test_tblite_task_gfn2xtb_m02(schema_versions, request):
    models, retver, _ = schema_versions
    ...
    return_result = np.array(
        [
            ...
            [-4.0644714892632226e-05, -7.6816568320453273e-03, +7.7872915896373372e-03],
...
# TODO
@uusing("tblite")
def hide_test_tblite_task_gfn2xtb_m03(schema_versions, request):
    models, retver, _ = schema_versions
...
@uusing("tblite")
def test_tblite_task_gfn2xtb_m04(schema_versions, request):
    models, retver, _ = schema_versions
...
@uusing("tblite")
def test_tblite_task_gfn2xtb_m05(schema_versions, request):
    models, retver, _ = schema_versions
...
#@uusing("tblite")
#def test_tblite_task_ipea1xtb_m01(schema_versions, request):
#    models, retver, _ = schema_versions
#
#    thr = 1.0e-8
#
#    return_result = -26.98270759167768
#
#    if from_v2(request.node.name):
#        atomic_input = models.AtomicInput(
#            molecule=models.Molecule(**qcng.get_molecule("mindless-05", return_dict=True)),
#            specification=models.AtomicSpecification(
#                model={"method": "IPEA1-xTB"},
#                driver="energy",
#            ),
#        )
#    else:
...
@uusing("tblite")
def test_ipea1_charge(schema_versions, request):
    models, retver, _ = schema_versions

    thr = 1.0e-6

    mol = {
        "symbols": np.array(["Si", "H", "H", "H", "H"]),
        "geometry": np.array(
          [
    ...
    assert atomic_result.success
    assert pytest.approx(-4.1365706907641115, abs=thr) == atomic_result.return_result
    ...
    assert atomic_result.success
    assert pytest.approx(-3.5168511494225148, abs=thr) == atomic_result.return_result


@uusing("tblite")
def test_tblite_task_unknown_method(schema_versions, request):
...
@uusing("tblite")
def test_tblite_task_unsupported_driver(schema_versions, request):
    models, retver, models_out = schema_versions
